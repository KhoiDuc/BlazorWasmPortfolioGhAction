...
public static class UnicodeData
{
    public static ICollection<CharInfoWrapper> GetData(string search)
    {
        ...
        if (search.StartsWith("\\u", StringComparison.OrdinalIgnoreCase) ||
            search.StartsWith("U+", StringComparison.OrdinalIgnoreCase) ||
            search.StartsWith("&#", StringComparison.OrdinalIgnoreCase))
        {
            var value = search[2..];
            if (int.TryParse(value, NumberStyles.AllowHexSpecifier, CultureInfo.InvariantCulture, out var code))
                return [CharInfoWrapper.Create(new Rune(code))];
        }

        if (int.TryParse(search, NumberStyles.AllowHexSpecifier, CultureInfo.InvariantCulture, out var hexCode))
            return [CharInfoWrapper.Create(new Rune(hexCode))];
