...
public sealed record CharInfoWrapper(UnicodeCharacterInfo CharInfo)
{
    ...
    private static string GetEscapeString(int value) =>
        char.ConvertFromUtf32(value).Length is 2
            ? "\\U" + value.ToString("X", CultureInfo.InvariantCulture).PadLeft(8, '0')
            : "\\u" + value.ToString("X", CultureInfo.InvariantCulture).PadLeft(4, '0');

    public static CharInfoWrapper Create(char c)
    {
        var info = Unicode.GetCharacterInfo(c);
        return info != null ? new CharInfoWrapper(info.Value) : new CharInfoWrapper(default(UnicodeCharacterInfo));
    }
