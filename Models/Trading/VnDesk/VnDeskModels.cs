...
public record BacktestTrade(string Symbol, DateTime Entry, decimal EntryPrice, DateTime? Exit, decimal? ExitPrice, decimal? TakeProfit, decimal? StopLoss, int HoldDays, decimal Pnl, double PnlPct, bool IsWin, decimal NetPnl = 0, double NetPnlPct = 0);
public record BacktestResult(string Indicator, int TotalTrades, int Wins, int Losses, double WinRate, double AvgTakeProfitPct, double AvgHoldDays, decimal TotalPnl, List<BacktestTrade> Trades, decimal TotalNetPnl = 0);
public record BacktestRequest(string Indicator, List<string> Symbols, DateTime From, DateTime To, decimal TakeProfitPct = 10, decimal StopLossPct = 7, decimal FeeRate = 0.0015m, decimal SellTaxRate = 0.001m, decimal SlippageBps = 0, decimal CapitalPerTrade = 100_000_000m);
